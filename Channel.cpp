...
void Channel::setI(bool value) {
    ...
    if (value) {
        std::cout << "Channel " << _name
                  << " set to invite only mode." << std::endl;
    } else {
        std::cout << "Channel " << _name
                  << " unset from invite only mode." << std::endl;
    }
}
...
void Channel::setT(bool value) {
    ...
    if (value) {
        std::cout << "Channel " << _name
                  << " set to topic settable by ops only." << std::endl;
    } else {
        std::cout << "Channel " << _name
                  << " unset from topic settable by ops only." << std::endl;
    }
}
...
void Channel::setK(bool value, const std::string &key) {
    ...
    if (value && !key.empty()) {
        _key = key;
        std::cout << "Channel " << _name
                  << " set to key protected mode with key: "
                  << _key << std::endl;
    } else if (!value) {
        _key = "";
        std::cout << "Channel " << _name
                  << " unset from key protected mode." << std::endl;
    }
}
...
void Channel::setL(bool value, int limit) {
    ...
    if (value && limit > 0) {
        _limit = limit;
        std::cout << "Channel " << _name
                  << " set to limited user count mode with limit: "
                  << _limit << std::endl;
    } else if (!value) {
        _limit = 0;
        std::cout << "Channel " << _name
